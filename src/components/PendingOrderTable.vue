...
import exportPendingOrder from "./../functions/pending-order-export";
...
const fetchPendingOrders = async (page = 1) => {
  ...
  try {
    const res = await axios.get(
      `${bUrl.value}/api/pending-orders?page=${page}&compId=${cCompany.value.id}`
    );
            ...
            <button
              ...
              @click="
                exportPendingOrder(
                  ...
                  null,
                  pendingOrders,
                  `Pending orders page - ${currentPage}`
                )
              "
            >
