...
import exportsupplier from "../functions/supplier-export";
...
const searchTxt = ref("");
const supplierRecords = ref(null);
...
const getInvRows = () => {
  if (props.type === "local") {
    supplierRecords.value = offlineRecords.value;
    supplierRecords.value = supplierRecords.value.map((rec) => {
    ...
    supplierRecords.value = records.value.data;
    supplierRecords.value = supplierRecords.value.map((rec) => {
      // let obj = {};
  ...
  Swal.fire({
    ...
    <div class="row">
      <div class="form-group col-md-6">
        ...
        <input type="date" class="form-control" id="supplierFrom" />
      </div>
      <div class="form-group col-md-6">
        <label for="">To</label>
        <input type="date" class="form-control" id="supplierTo" />
      </div>  
    </div>
   `,
    focusConfirm: false,
    preConfirm: () => {
      return [
        document.getElementById("supplierFrom").value,
        document.getElementById("supplierTo").value,
      ];
    },
  }).then(async (result) => {
    if (result.isConfirmed) {
      ...
      let url = `
      filter?model=supplier&from=${val[0]}&to=${val[1]}&compId=${cCompany.value.id}
      `;
