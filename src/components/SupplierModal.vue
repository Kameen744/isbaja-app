...
let supplierLocaRecModal = null;

const toggleLocalRecModal = (t = false) => {
  if (t) {
    supplierLocaRecModal.show();
  } else {
    supplierLocaRecModal.hide();
  }
};
...
onMounted(() => {
  supplierLocaRecModal = new bootstrap.Modal(modalRef.value);
});
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <supplierTable
            :tabCols="tabCols"
            :secName="secName"
            :key="`supplier-local-${offlineRecords?.length}`"
            type="local"
            v-if="offlineRecords"
          />
        </div>
